        ...
        div(style='width=100%')
            div(class:hidden!='{data === "Não autorizado"}' style!='{geraCSS(style)}')
                span {title} (Approx. {Math.floor(data.split(";base64,")[1].length/4 * 3 / 1024)} kB)
                    br
                    button.action.button(class:hidden!='{!is_image}' on:click!='{()=>preview.showModal()}') Visualizar
                    a(class:hidden!='{is_image}' href!='{data}' download!='{title}'): button.action.button {download}
                    Dialog(title='' bind:dialog!='{preview}')
                        .file-wrapper
                            img(class:hidden!='{!is_image}' '{alt}' src!='{data}')
                            object(class:hidden!='{data.split(";base64,")[1].length < 20 || is_image}' '{alt}' '{data}' '{title}') Não pudemos exibir
                            div
                                a.action.button(class:hidden!='{!(data && download)}' href!='{data}' download!='{title}') {download}
